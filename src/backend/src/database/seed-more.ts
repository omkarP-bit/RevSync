...
// ---------------------------------------------------------------------------
// Supplemental seeder — ADDS volume data on top of the existing database.
// It does NOT truncate. Safe to run repeatedly? Yes, it uses explicit numbers,
// but re-running will duplicate rows. Intended as a one-time bulk data load.
// ---------------------------------------------------------------------------
...
async function seedMore() {
  ...
  await withTransaction(async (client) => {
    ...
    const bronzeId = tierId("Bronze");
    const silverId = tierId("Silver");
    const goldId = tierId("Gold");
    ...
    const quoteById = (id: number) => savedQuotes.find((q) => q.id === id)!;
...
seedMore().catch((err) => {
  console.error("Supplemental seeding failed:", err);
  process.exit(1);
});