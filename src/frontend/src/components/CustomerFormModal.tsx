...
export function CustomerFormModal({ isOpen, onClose, onSuccess }: CustomerFormModalProps) {
  ...
  // Created Customer Setup State (Success View)
  ...
  const setupUrl = createdCustomer?.setup_token ? `${baseUrl}/portal/setup?token=${createdCustomer.setup_token}` : "";
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-start pb-4 border-b border-slate-100">
              <div>
                ...
                <h2 className="text-xl font-extrabold text-slate-900 tracking-tight mt-2">
                  Customer Portal Setup Link
                </h2>
                <p className="text-xs text-slate-500 mt-0.5">
                  Share this single-use link with the customer to activate their Customer Portal account.
                </p>
              </div>
              <button onClick={handleClose} className="text-slate-400 hover:text-slate-700 font-bold p-1 text-lg">
                ✕
              </button>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-900">{createdCustomer.name} {createdCustomer.company ? `(${createdCustomer.company})` : ""}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Email:</span>
                <span className="font-bold text-slate-900">{createdCustomer.email}</span>
              </div>
            ...
            {setupUrl && (
              <div className="space-y-3">
                <label className="block text-xs font-bold text-slate-700">
                  Customer Portal Setup Link (Single-Use • Expires in 24h)
                </label>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    readOnly
                    value={setupUrl}
                    className="w-full bg-slate-100 border border-slate-300 rounded-xl px-3.5 py-2.5 text-xs font-mono text-slate-800 focus:outline-none select-all"
                  />
                </div>

                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-[11px] text-amber-800 font-medium leading-relaxed">
                  🔒 <strong>Security Note:</strong> Passwords are never displayed. The customer must access this link to choose their password securely.
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    ...
                    className="flex-1 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs shadow-xs transition-colors flex items-center justify-center gap-1.5"
                  >
                    {copied ? (
                      <>
                        <span>✓</span>
                  ...
                  <button
                    ...
                    className="py-3 px-4 bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl text-xs transition-colors"
                  >
                    Open Portal Setup
                  </button>
                </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button
                type="button"
                onClick={handleClose}
                className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-extrabold rounded-xl text-xs shadow-xs transition-colors"
              >
                Done & Continue to Quotation
              </button>
            </div>
          </div>
        ) : (
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="mt-4 space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                >
                  {loading ? "Creating Customer..." : "Create Customer & Generate Setup Link"}
                </button>
              </div>
            </form>
          </div>
